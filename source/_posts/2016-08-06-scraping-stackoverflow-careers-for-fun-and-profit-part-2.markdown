...
The source code for this project can be found in [stackjobs repo](https://github.com/gyurisc/stackjobs) on github.

### Installing mongodb

I need to store the data somewhere and I choose to use mongodb, because of no particular reason :). Let's install it via the [homebrew](http://brew.sh/) package manager by using the commands: 
...
If you want to download the install package or use some other methods to install, please visit the page [Install MongoDB Community Edition on OS X](https://docs.mongodb.com/master/tutorial/install-mongodb-on-os-x/) and choose the method that suits you.
...
```
You can confirm that the mongodb service is up and running by visiting [http://localhost:28017](http://localhost:28017).
...
``` bash Starting and stopping mongodb servce using brew.
...
``` bash Exporting data to csv file.
...
``` bash Exporting data in json format. 
...
``` bash Listing existing crontab jobs 
crontab -l 
``` 
### Summary 
